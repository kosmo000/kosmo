...
import Chapter07.EX02.A;

class A {	//클래스 : 객체를 생성하기 위한 설계도
	//클래스 내부에 올수 있는것(4가지) : 1. 필드(클래스에서 선언된변수), 2. 생성자, 3. 메소드, 4.인너클래스
	//1. 필드
	...
	//		클래스 내부의 다른 생성자가 존재할시 컴파일럭가 기본 생성자를 자동으로 만들어 주지 않는다.
	A () {
...
public class Using_Object {
	...
	public static void main(String[] args) {
		...
		//A() : 클래스 내부의 생성자 호출, 기본생성자 : 입력 매개변수가 없는 생성자 , 생략될수 있다.
		...
		//메소드 호출 (work2 (정수, 정수))
		int k = a1.work2(10,20);	//매개변수 가 2개인 메소드 호출 << return a + b;
		...
		// 인스턴스화 : 클래슨느 객체화 해야 사용 가능 하다. A a = new A();
		A a2 = new A();
		A a3 = new A();
		A a4 = new A();
		A a5 = new A();
