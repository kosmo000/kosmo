...
class Person{
	...
	//1. 필드 : 클래스 블락에서 선언된 변수, 지역변수 : 메소드 블락에서 선언된 변수 (stack)
	String name ;	// 이름			: null
	float height;	// 키 (실수)		: 0.0
	float weight;	// 몸무게 (실수)	: 0.0
	int age ;		// 나이			: 0
	
	//2. 생성자 (Constructor) : 1. 리턴타임이 없다. 2. 클래스 이름과 동일 (메소드)
			//필드의 기본값을 초기화
...
public class Using_Object_review {
	...
	public static void main(String[] args) {
		...
		//3. 매개변수가 4개인 생성자 호출
		System.out.println("== 매개변수 4개 생성자 출력==");
		Person hong = new Person("홍길동", 185.5F,80.3F, 35);
		System.out.println(hong.name);		//홍길동
		System.out.println(hong.weight);	//185.5
		System.out.println(hong.height);	//80.3
		System.out.println(hong.age);		//35
	
		hong.show();
		
		//Setter 를 사용해서 필드의 값할당, Getter를 사용해서 필드의 값출력
		
		System.out.println("==Setter/Getter 사용 ==");
		Person jung = new Person();
		
		//setter 를 사용해서 필드가 값할당.
		jung.setName("정도령");
