...
public class Using_Method_Overriding02 {
	...
	public static void main(String[] args) {
		...
		bb.cry();	//bb.cry() : Animal의 cry()를 호출
		cc.cry();	//cc.cry() : Animal의 cry()를 호출
		dd.cry();	//dd.cry() : Animal의 cry()를 호출
		
		System.out.println("================");
		
		//2. Animal 타입으로 선언 + 자식 클래스로 객체 생성
			//자식의 객체를 생성할때 부모 타입으로 정의후 부모의 메소드를 호출 시 오버라이딩된 메소드가 출력
		...
		bb1.cry();	//bb1.cry() : Animal 의 cry()를 호출 ==> 오버라이딩된 메소드가 출력
		cc1.cry();	//cc1.cry() : Animal 의 cry()를 호출 ==> 오버라이딩된 메소드가 출력
		dd1.cry();	//dd1.cry() : Animal 의 cry()를 호출 ==> 오버라이딩된 메소드가 출력
