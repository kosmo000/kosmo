...
public class PanelDrk extends JPanel { // JPanel에 PanelDrk 상속
	
	MainTest parent; //MainTest 객체 생성

	// 1. 멤버변수 선언
	JButton[] bts;		// 반복문 사용을 위해 버튼을 배열로 선언
	String[] menu = new String[] {"콜라", "사이다", "레몬에이드", "청포도에이드", "아메리카노", "오렌지주스"};
	// 메뉴에 넣을 내용을 배열을 통해 반본문으로 선언
...
//	ArrayList<MenuVO> list = new ArrayList<MenuVO>();
	...
	public PanelDrk (MainTest mainTest) { // PanelDrk 객체 생성
	...
	void eventProc() {
		...
		for ( int i = 0; i < bts.length; i++ ) { 	// 반복문 선언
			...
			bts[i].addActionListener(new ActionListener() {	// 어느버튼을 눌렀을 때 나오는 것
